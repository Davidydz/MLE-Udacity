...
human_faces1 = 0
human_faces2 = 0
for h_f in human_files_short:
    human_faces1 += face_detector(h_f)
for d_f in dog_files_short:
    human_faces2 += face_detector(d_f)
human_faces1 /= 100
human_faces2 /= 100
print("The ratio of human faces in the human pictures dataset is {:.2f}".      format(human_faces1))
print("The ratio of human faces in the dog pictures dataset is {:.2f}".     format(human_faces2))
...
# __回答:__
# 
# 一般来说，在需要用户照片的网站上都会有要求用户照片中人脸清晰的要求，如果人脸不清晰系统会返回照片让用户重新上传，这在一定程度上保证了最终照片的清晰度。如果没有这样的要求，我们假设用户随意上传的人像照片，那人像清晰的要求就显得不太合理，用户可能因为照相时手部抖动或者像素等原因导致其提供的面部特征图像比较模糊。为了解决这种问题，我认为可以在训练集中加入模糊照片，使得神经网络从训练集中学会识别模糊面部特征的人脸图像。
...
dogs2 = 0
for h_f in human_files_short:
    dogs1 += dog_detector(h_f)
for d_f in dog_files_short:
    dogs2 += dog_detector(d_f)
dogs1 /= 100
dogs2 /= 100
print("The ratio of dogs in the human pictures dataset is {:.2f}".      format(dogs1))
print("The ratio of dogs in the dog pictures dataset is {:.2f}".     format(dogs2))
...
# 
# __回答:__  
# 我的模型结构是卷积层+卷积层+最大化池层+卷积层+最大化池层+Dropout层+Flatten层+全连接层+Dropout层+softmax层。  
# 首先使用两个卷积层加一个最大化池层是希望模型能够在局部上总结出一些有用的特征，然后将局部明显的特征给提取出来。之后又用了一个卷积层，而且kernel_size相对较大，因为希望能够把之前局部的明显特征给连接起来。在这之后又用了一个最大化池层来提取明显的特征。之后连接了一个Flatten层和两个全连接层，这样做的目的是因为我想利用所有总结出来的信息，并通过其归纳出最后图片的分类。中间添加的Dropout层是为了防止过拟合。
...
### TODO: 定义你的网络架构
model.add(Conv2D(filters = 10, kernel_size = 3, padding = "same",                  activation = "relu", input_shape = (224,224,3)))
model.add(Conv2D(filters = 8, kernel_size = 3, activation = "relu"))
model.add(MaxPooling2D(pool_size = 3))
#model.add(Dropout(0.2))
model.add(Conv2D(filters = 6, kernel_size = 10,                  activation = "relu"))
...
# 
# __回答:__   
# 1. 我的神经网络结构为：Xception + GlobalAveragePooling层 + softmax层  
# 2. 在尝试了另外三个神经网络架构之后，Xception在validation set上的表现略微优于InceptionV3和Resnet50,并且明显优于VGG19，因此神经网络的基本架构我选择了Xception。  
# 3. 由于我们的训练数据集不大，因此第一层用GlobalAveragePooling层以减少维度。然后我使用softmax层对各个种类的概率进行预测，其原因是我们的数据集不大，而且Xception已经高度总结了图片特征，我们只需要简单将这些特征整合即可。
# 4. VGG16与VGG19的表现不如Xception, Resnet50, 以及InceptionV3的原因，我认为应该是和其神经网络架构有关。相比于其余三个， VGG的架构更加简单，因此能够提取的特征也就更加有限。
# 5. 对于Xception架构本身成功的原因，我认为是其复杂的架构，它为提取复杂图片特征提供了保障。
# 
# 
...
#VGG19_model.add(Dense(152,activation = "sigmoid"))
...
Xception_model.add(GlobalAveragePooling2D(input_shape = train_model.shape[1:]))
Xception_model.add(Dropout(0.6))
...
def predict_dog_breed(img_path):
    ...
    breed = np.argmax(predicted_vector)
    return dog_names[breed].split(".")[1]
...
def my_algo(img_path):
    ...
    plt.imshow(img)
    if faces > 0 and not dogs:
        print("hello human!")
        dog_breed = predict_dog_breed(img_path)
        print("You look like a " + dog_breed)
    elif dogs:
        dog_breed = predict_dog_breed(img_path)
        print("Hello "+ dog_breed)
    else:
        print("Ennn...I can only figure out human and dogs")
...
# 2. 提出至少三点改进你的模型的想法。
# 
# 1.输出的结果比我预想的稍微差了一点。比如最后一张其实是柴犬的照片，但是被识别为人类了。
# 2.关于模型的改进我有以下三点想法：  
#     a. 在最后一张柴犬的识别中，可以看出Xception模型猜出了它是柴犬，但是Resnet50以及Casecade分类器并没有将其识别为狗狗。因此在transfer learning的训练集中或许可以尝试加入不是狗狗类别的照片，让模型能够判断狗狗与不是狗狗的类别。  
#     b. 增加训练集的大小。可以看出Xception模型最后没能猜对柴犬，这跟照片旋转，反转，或者放缩无关，因此我个人认为应该从增加训练集大小，引入更多种类的狗狗照片这方面进行。  
#     c.使用Transfer Learning的办法优化Resnet50，使其更好地区分狗狗和人类。
...
my_algo("alasijia.jpg")


# In[105]:


my_algo("chaiquan.jpg")
